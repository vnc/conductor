	...
	try {
		ec2.call('RunInstances', createParams, function(result) {
			...
			if (result.instancesSet) {
				...
				msg.data = { message: instanceId + ' is now ' + status, user: username };
...
router.get('/ec2/*', function(req, res, action) {
	...
	var authorizedCallback = function(username) {
	...
	auth.authenticate(req, exports.config, notAuthorizedCallback, authorizedCallback);
})
.notFound(notFound);
